    ...
    public String submitForm(@Valid @ModelAttribute("product") Product product, BindingResult bindingResult, Model theModel) {
        ...
        if (bindingResult.hasErrors()) {
            ...
            if (product.getId() != 0) {
                ...
                int difference = product.getInv() - product2.getInv();
                if (difference > 0) {
                    for (Part p : product2.getParts()) {
                        int newInv = p.getInv() - difference;
                        if (newInv < p.getMinInv()) {
                            theModel.addAttribute("errorMessage", "Updating this product would lower the inventory of part '" + p.getName() + "' below its minimum inventory of " + p.getMinInv() + ".");
